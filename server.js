...
// --- 1. SETTINGS MODEL (Isko hamesha routes se upar rakho) ---
const settingsSchema = new mongoose.Schema({
    slot1Time: { type: String, default: "TBA" },
    slot2Time: { type: String, default: "TBA" },
    slot3Time: { type: String, default: "TBA" }, 
    slot4Time: { type: String, default: "TBA" }, 
...
app.post('/register', upload.single('screenshot'), async (req, res) => {
    try {
        ...
        if (totalTeams >= (LIMIT * 4)) { 
            return res.status(400).json({ success: false, message: "All 4 Slots (100 Teams) are Full!" });
        }
        ...
        let assignedSlot = Math.floor(totalTeams / LIMIT) + 1;
        ...
        const newTeam = new Team({
            ...
            player3Name: req.body.player3Name, 
            player4Name: req.body.player4Name, 
...
// --- 4. SETTINGS ROUTES (Fixed for 4 Slots) ---
...
app.post('/admin/update-schedule', async (req, res) => {
    const { slot1, slot2, slot3, slot4 } = req.body; // Slot 3 and 4 added
...
app.get('/get-status', async (req, res) => {
    try {
        ...
        res.json({ 
            ...
            schedule3: settings.slot3Time, // Added
            schedule4: settings.slot4Time, // Added
